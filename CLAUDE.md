...
4. **One search serves everyone, per storefront.** A single store search per keyword (a keyword is scoped by `text, store, country`) yields positions for the primary app and all its competitors in that storefront. Never one search per app. The same keyword tracked in two countries is two keyword rows checked by two searches — rankings differ per storefront. Each added country multiplies daily search volume against the same `SCRAPE_ITUNES_RPM` budget; `GET /jobs/budget` estimates the daily fan-out and the settings budget card surfaces it.
